...
Namespace GetCheckboxCheckedValue
    ...
    Friend Class Program
        ...
        Shared Sub Main(ByVal args As String())
            ...
            formData("Female").Value = GetCheckboxCheckedValue.Program.GetCheckboxCheckedValue(processor, "Female")
        ...
        Private Shared Function GetCheckboxCheckedValue(ByVal processor As PdfDocumentProcessor, ByVal fieldName As String) As String
            If processor.Document Is Nothing OrElse processor.Document.AcroForm Is Nothing Then Return Nothing
            Dim button As PdfInteractiveFormField = GetCheckboxCheckedValue.Program.FindCheckBox(processor.Document.AcroForm.Fields, "", fieldName)
            If button IsNot Nothing Then
                ...
                If appearance IsNot Nothing Then
                    ...
                    If names.Count = 1 Then Return names.First()
                    If names.Count > 1 Then Return If(Equals(names(0), "Off"), names(1), names(0))
                End If
        ...
        Private Shared Function FindCheckBox(ByVal fields As IEnumerable(Of PdfInteractiveFormField), ByVal partialName As String, ByVal fieldName As String) As PdfInteractiveFormField
            For Each field As PdfInteractiveFormField In fields
                If Equals(partialName & field.Name, fieldName) Then
                    If field.Flags.HasFlag(PdfInteractiveFormFieldFlags.Radio) OrElse field.Flags.HasFlag(PdfInteractiveFormFieldFlags.PushButton) OrElse field.Widget Is Nothing Then Return Nothing
                    Return TryCast(field, PdfButtonFormField)
                End If

                If field.Kids IsNot Nothing Then
                    Dim intermediateResult As PdfInteractiveFormField = GetCheckboxCheckedValue.Program.FindCheckBox(field.Kids, partialName & "." & field.Name, fieldName)
                    If intermediateResult IsNot Nothing Then Return intermediateResult
                End If
            Next
